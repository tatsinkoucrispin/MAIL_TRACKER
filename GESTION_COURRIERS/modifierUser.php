<?php
    ...
    if(isset($_POST['modifierUser'])){
        ...
        if($user==null || $typeC==null){
            echo "<div class='alert alert-danger almessage' role='alert'>
            Veuillez remplir les champs obligatoires!
            </div>";
        }else{
            $depsql = "UPDATE utilisateur SET NOMUTILISATEUR=:nom,TYPECOMPTE=:typec WHERE IDCOMPTE='$id'";
				...
				<div class="content-data">
					<div class="details">
						<div class="recentOrders">
                            ...
                            <?php 
                                $sql = "SELECT * FROM utilisateur where IDCOMPTE = '$id'";
                                $exUser = $conn->prepare($sql);
                                $exUser->execute();
                                $tabUser=$exUser->fetchAll(PDO::FETCH_ASSOC);
                                foreach($tabUser as $value){
                                ?>

                            <div class="mb-3">
                                <label>Nom d'utilisateur :</label>
                                <input type="text" value="<?php echo $value['NOMUTILISATEUR']?>" name="user" class="form-control">
                            </div>
        
                            <div class="col mb-4">
                                <label>Type :</label>
                                <select name="typeC" class="form-control">
                                    <option value="" selected disabled hidden>Type de compte</option>
                            ...
                            <?php }?>
                                <div class="mb-4">
                                    <button type="submit" name="modifierUser" class="btn btn-success btn-lg">modifier</button>
