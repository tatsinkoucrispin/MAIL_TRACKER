<?php
    ...
    if(isset($_POST['ajDep'])){
        $ndep = filter_var(htmlentities($_POST['ndep']),FILTER_SANITIZE_STRING);
        $rsp = filter_var(htmlentities($_POST['rsp']),FILTER_SANITIZE_STRING);

        $sql = "SELECT * FROM departement WHERE NOM_DEPARTEMENT=:ndep";
        $exist_dep = $conn->prepare($sql);
        $exist_dep->bindParam(':ndep', $ndep, PDO::PARAM_STR);
        $exist_dep->execute();
        $num_dep = $exist_dep->rowCount();
        if($num_dep == 1){
            echo "<div class='alert alert-danger almessage' role='alert'>
            Departement existe déjà!
             </div>";
        }
        else if($ndep==null || $rsp==null){
            echo "<div class='alert alert-danger almessage' role='alert'>
            Veuillez remplir les champs obligatoires!
             </div>";
        }else{
            $depsql = "INSERT INTO departement (NOM_DEPARTEMENT,RESPONSABLE)
                VALUES( :ndep, :rsp)";
                $query = $conn->prepare($depsql);
                $query->bindParam(':ndep', $ndep, PDO::PARAM_STR);
					...
					<div class="details">
						<div class="recentOrders">
							<div class="cardHeader">
								<h2>Ajout d'un département</h2><br>
							</div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="ndep" class="form-control">
                            </div>
                            <div class="mb-3">
                                ...
                                <div class="mb-4">
                                    <button type="submit" name="ajDep" class="btn btn-success btn-lg">Ajouter</button>
