<?php
    ...
    if(isset($_POST['ajouterDes'])){
        $nom = filter_var(htmlentities($_POST['nom']),FILTER_SANITIZE_STRING);
        $adresse = filter_var(htmlentities($_POST['adresse']),FILTER_SANITIZE_STRING);
        $tele = $_POST['tele'];
        $fax = $_POST['fax'];

        $sql = "SELECT * FROM destinataire WHERE NOM_DES=:nom";
        $exist_exp = $conn->prepare($sql);
        $exist_exp->bindParam(':nom', $nom, PDO::PARAM_STR);
        $exist_exp->execute();
        $num_exp = $exist_exp->rowCount();
        if($num_exp == 1){
            echo "<div class='alert alert-danger almessage' role='alert'>
            Destinataire existe déjà!
             </div>";
        }
        else if($nom==null || $adresse==null){
            echo "<div class='alert alert-danger almessage' role='alert'>
            Veuillez remplir les champs obligatoires!
             </div>";
        }else{
            $expsql = "INSERT INTO destinataire (NOM_DES,ADRESSE_DES,TEL_DES,FAX_DES)
                VALUES( :nom, :adresse, :tele, :fax)";
					...
					<div class="details">
						<div class="recentOrders">
							<div class="cardHeader">
								<h2>Ajout d'un destinataire</h2><br>
							</div>
