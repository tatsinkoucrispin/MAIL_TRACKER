<?php
    ...
    if(isset($_POST['mPer'])){
        ...
        if($nomP==null || $iddep==null){
            echo "<div class='alert alert-danger almessage' role='alert'>
            Veuillez remplir les champs obligatoires!
             </div>";
        }else{
            $expsql = "UPDATE personnel SET ID_DEPARTEMENT=:iddep, NOM_PERSONNEL=:nomp ,ADRESSE_PERSONNEL=:adressep,TEL_PERSONNEL=:tele WHERE ID_PERSONNEL = '$id'";
					...
					<div class="details">
						<div class="recentOrders">
							<div class="cardHeader">
								<h2>Modification du personnel</h2><br>
							</div>
		
                            <form action="" method="POST">
                            <?php 
                                $sql = "SELECT * FROM personnel where ID_PERSONNEL = '$id'";
                                $pers = $conn->prepare($sql);
                                $pers->execute();
                                $tabPers=$pers->fetchAll(PDO::FETCH_ASSOC);
                                foreach($tabPers as $value){
                            ?>
                            <div class="mb-3">
                                <label>Nom :</label>
                                <input type="text" value="<?php echo $value['NOM_PERSONNEL']?>" name="nomP" class="form-control">
                            </div>
                            <div class="mb-3">
                                <label>Adresse :</label>
                                <input type="text" value="<?php echo $value['ADRESSE_PERSONNEL']?>" name="adresseP" class="form-control">
                            </div>
                            <div class="mb-3">
                                <label>N&#186; tele :</label>
                                <input type="text" value="<?php echo $value['TEL_PERSONNEL']?>" name="tele" class="form-control">
                            </div>
                            <?php }?>
                            <div class="mb-5">
                            ...
                            <div class="mb-4">
                                <button type="submit" name="mPer" class="btn btn-success btn-lg">Modifier</button>
